...
export default function RepoSwitcher() {
  ...
  return (
    <DropdownMenu onOpenChange={(v) => { if (!v) setShowPicker(false); }}>
      ...
      <DropdownMenuContent className="w-56">
        {repos.map((r) => (
          <DropdownMenuItem key={r.id} onClick={() => selectRepo(r.id)}>
            {r.name}
          </DropdownMenuItem>
        ))}
        <DropdownMenuSeparator />
        {!showPicker && (
          <DropdownMenuItem onSelect={(e) => { e.preventDefault(); setShowPicker(true); }}>
          ...
          <>
            ...
            {selectable.map((repo) => (
              <DropdownMenuItem
                ...
                onSelect={(e) => { e.preventDefault(); handleConnect(repo); }}
