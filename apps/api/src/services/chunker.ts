...
// Chunks that carry no retrievable meaning on their own — pure import
// blocks, re-export lines, or a bare "export default Name;" — add noise
// to search results without ever being the right answer to a question.
...
function isLowValueChunk(content: string): boolean {
  ...
  return lines.every((l) => LOW_VALUE_LINE.test(l));
}
...
export async function chunkRepo(rootDir: string): Promise<CodeChunkResult[]> {
  ...
  return chunks.filter((c) => !isLowValueChunk(c.content));
}
