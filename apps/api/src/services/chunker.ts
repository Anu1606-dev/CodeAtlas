...
const TOP_LEVEL_BOUNDARY =
  /^(export\s+)?(default\s+)?(async\s+)?(function\b|class\b|interface\b|type\s+\w+\s*=|const\s+\w+\s*=\s*(async\s*)?\(|const\s+\w+\s*=\s*(async\s*)?function)/;
...
function chunkJsTsFile(content: string): RawChunk[] {
  ...
  while (i < lines.length) {
    ...
    if (!/^\s/.test(line) && TOP_LEVEL_BOUNDARY.test(line)) {
      ...
      const startLine = i;
      const symbolMatch = line.match(/\b(function|class|interface|type|const)\s+(\w+)/);
